...
export class EmartService {
  ...
  getAllItems() {
    return this.http.get("http://localhost:4200/emart/item/all");
  }
  ...
  getItem(itemId) {
    return this.http.get("http://localhost:4200/emart/item/" + itemId);
  }
...
//----Storing BillDetails----//
  addBill(todayDate: Date, total: number) {
    ...
    this.cartItems = [];
    return this.http.post("http://localhost:4200/emart/bill", bill);
  }

  //----Retrieving Current Buyer Data----//
  getBuyer() {
    return this.http.get("http://localhost:4200/emart/buyer/" + this.currentBuyer.id);
  }
  ...
  validateBuyer(user: string, password: string) {
    ...
    return this.http.get("http://localhost:4200/emart/validate", { headers });
